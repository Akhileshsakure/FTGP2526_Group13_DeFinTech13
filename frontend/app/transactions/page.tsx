  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError((err as Error).message || "Could not load transaction history");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="fade-up">
      <div className="mb-8">
        ...
        <p className="text-stone-500 text-sm">
          Your protocol activity with amounts, addresses, hashes, and block details
        </p>
      </div>

      {!isConnected && (
        <div className="card p-10 text-center">
          <div className="font-display text-2xl mb-2 text-stone-400">Connect your wallet</div>
          <p className="text-stone-500 text-sm">
            Connect MetaMask to view your personal transaction history.
          </p>
        </div>
      )}
      ...
      {isConnected && isCorrectNetwork && (
        <div className="card overflow-hidden">
          <div className="px-5 py-3 border-b border-stone-100 flex items-center justify-between gap-3">
            <span className="font-medium text-sm text-stone-700">Personal Records</span>
          ...
          {loading ? (
            ...
            <div className="px-5 py-12 text-center">
              ...
              <p className="text-stone-500 text-sm">
                Supply, borrow, repay, or change collateral settings to create records here.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-stone-100">
...
function Detail({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded border border-stone-200 bg-white p-3">
      ...
      <div className={`${mono ? "font-mono text-xs break-all" : "text-sm"} text-stone-700`}>
        {value}
      </div>
    </div>
  );
